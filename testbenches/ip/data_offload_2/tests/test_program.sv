...
program test_program;
  ...
  int                                src_transfers_initial_count;
  int                                src_transfers_delay;
  int                                src_transfers_delayed_count = 1;
  ...
  initial begin
    ...
    src_transfers_initial_count = $urandom_range(10, 1);
    ...
    time_to_wait                = (`MEM_TYPE == 2) ? 40000 : 10000;

    `INFO(("Randomized stimulus: length=%0d initial_count=%0d delay=%0d dst_ready_mode=%s time_to_wait=%0d",
      src_transfers_length, src_transfers_initial_count, src_transfers_delay,
      dst_ready_mode.name(), time_to_wait), ADI_VERBOSITY_LOW);

    // Configure environment sequencers
    test_env.configure(
      .transfer_length(src_transfers_length),
      .transfer_count(src_transfers_initial_count),
      .path_type(`PATH_TYPE),
      .dst_ready_mode(dst_ready_mode),
      .dst_ready_high(dst_ready_high),
      .dst_ready_low(dst_ready_low),
      .oneshot(`OFFLOAD_ONESHOT));
    ...
    if (!`OFFLOAD_ONESHOT) begin
      test_env.src_axis_agent.master_sequencer.wait_empty_descriptor_queue();
      test_env.init_req_vip_if.set_io(1'b0);
    end
    ...
    for (int i = 0; i < src_transfers_delayed_count; i++) begin
      test_env.src_axis_agent.master_sequencer.add_xfer_descriptor_byte_count(src_transfers_length, `PATH_TYPE, 0);
    end
