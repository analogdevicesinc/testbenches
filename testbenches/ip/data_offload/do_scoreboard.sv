          ...
          for (int i=0; i<(transaction.get_len()+1); i++) begin
            ...
            for (int j=0; j<num_bytes; j++) begin
              ...
              if (transaction.get_cmd_type() == 1'b1) begin  // WRITE
                `INFO(("Caught a DDR WRITE transaction: %d -- %d", data_beat[j*8+:8], this.rx_sink_byte_stream_size), ADI_VERBOSITY_DEBUG);
                this.rx_sink_byte_stream.push_back(data_beat[j*8+:8]);
                this.rx_sink_byte_stream_size++;
              end else begin  // READ
                `INFO(("Caught a DDR READ transaction: %d -- %d", data_beat[j*8+:8], this.tx_source_byte_stream_size), ADI_VERBOSITY_DEBUG);
    ...
    task get_tx_sink_transaction();
      ...
      forever begin
        if (this.tx_sink_axis_ap.get_item_cnt() > 0) begin
          `INFO(("Caught a TX AXI4 stream transaction: %d", this.tx_sink_axis_ap.get_item_cnt()), ADI_VERBOSITY_DEBUG);
    ...
    task get_rx_source_transaction();
      ...
      forever begin
        if (this.rx_source_axis_ap.get_item_cnt() > 0) begin
          `INFO(("Caught a RX AXI4 stream transaction: %d", this.rx_source_axis_ap.get_item_cnt()), ADI_VERBOSITY_DEBUG);
    ...
    task compare_rx_transaction();
      ...
      forever begin : rx_path
        if ((this.rx_source_byte_stream_size > 0) &&
          ...
          this.rx_sink_byte_stream_size--;
          `INFO(("Scoreboard RX sink/source data: exp %h - rcv %h", source_byte, sink_byte), ADI_VERBOSITY_DEBUG);
    ...
    task compare_tx_transaction();
      ...
      forever begin : tx_path
        if ((this.tx_source_byte_stream_size > 0) &&
          ...
          this.tx_sink_byte_stream_size--;
          `INFO(("Scoreboard TX sink/source data: exp %h - rcv %h", source_byte, sink_byte), ADI_VERBOSITY_DEBUG);
          if (source_byte !== sink_byte) begin
            `INFO(("TX Scoreboard failed at: exp %h - rcv %h", source_byte, sink_byte), ADI_VERBOSITY_DEBUG);
            this.tx_error_cnt++;
      ...
      if (this.enabled == 0) begin
        `INFO(("Scoreboard was inactive."), ADI_VERBOSITY_DEBUG);
          ...
          if (tx_comparison_cnt == 0) begin
            ...
            `INFO(("TX scoreboard has passed. %d bytes were checked.", this.tx_comparison_cnt), ADI_VERBOSITY_DEBUG);
            `INFO(("TX transfer size are %d bytes.", this.tx_all_transfer_size), ADI_VERBOSITY_DEBUG);
          end
      end
    ...
    function void post_rx_test();
      if (this.enabled == 0) begin
        ...
        if (rx_comparison_cnt == 0) begin
          ...
          `INFO(("RX scoreboard has passed. %d bytes were checked.", this.rx_comparison_cnt), ADI_VERBOSITY_DEBUG);
        end
      end
