...
interface spi_vip_if #(
  int MODE              = 0,
      ...
      NUM_OF_MISO       = 1,
      NUM_OF_MOSI       = 1,
      MISO_LANE_MASK    = 8'hFF,
      MOSI_LANE_MASK    = 8'hFF,
      SLAVE_TIN         = 0,
      SLAVE_TOUT        = 0,
      MASTER_TIN        = 0,
      MASTER_TOUT       = 0,
  ...
  logic [NUM_OF_MOSI-1:0] m_mosi;
  logic m_cs;

  // internal
  spi_mode_t spi_mode;
  logic miso_oen;
  logic [NUM_OF_MISO-1:0] miso_drive;
  logic [NUM_OF_MISO-1:0] miso_drive_delayed;
  logic [NUM_OF_MOSI-1:0] mosi_drive = 1'b0;
  logic cs_drive = 1'b0;
  logic sclk_drive = 1'b0;
  logic cs_active;
  logic [NUM_OF_MOSI-1:0] s_mosi_delayed;
  ...
  class adi_spi_vip_if #(int dummy = 10) extends adi_spi_vip_if_base;
    ...
    function new();
      s_mosi_unpacked = new [NUM_OF_MOSI];
    endfunction
    ...
    virtual function int get_param_NUM_OF_MISO();
      return NUM_OF_MISO;
    endfunction

    virtual function int get_param_NUM_OF_MOSI();
      return NUM_OF_MOSI;
    endfunction

    virtual function int get_param_MISO_LANE_MASK();
      return MISO_LANE_MASK;
    endfunction

    virtual function int get_param_MOSI_LANE_MASK();
      return MOSI_LANE_MASK;
    endfunction
