...
The common steps for both test programs are:

* Configure the Data Offload
* Start the sequencers and scoreboard

Depending on the test program, the transfers are triggered and verified
differently.

test_program: back-to-back transfers
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* Assert the *init_req* and send an initial set of transfers of incremental data
* After a delay, assert the *init_req* again and send a second set of transfers
* Verify all transfers back-to-back
* Wait for scoreboard to complete checking

test_program_sync: externally-synchronized transfers
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* Assert the *init_req* to arm the offload
* Release one transfer per external sync pulse and verify all
* Wait for scoreboard to complete checking
