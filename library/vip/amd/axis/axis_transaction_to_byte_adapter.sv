  ...
  class axis_transaction_to_byte_adapter extends adi_component;
    ...
    protected task adapt_and_publish(
      ...
      while (data_fifo_in.size()) begin
        ...
        for (int i=0; i<axis_transaction_in.BYTES_PER_TRANSACTION; i++) begin
          if (axis_transaction_in.EN_TKEEP && axis_transaction_in.bytes[i].tkeep == 1'b1 && (!axis_transaction_in.EN_TSTRB || axis_transaction_in.bytes[i].tstrb == 1'b1)) begin
