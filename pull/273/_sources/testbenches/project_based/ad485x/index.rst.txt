...
.. note::
   ...
   The following properties are automatically determined by the selected
   DEVICE and are not configuration parameters:

   -  Number of channels: 4 channels for AD4851-AD4854, 8 channels for
      AD4855-AD4858
   -  Resolution: 16-bit for AD4851/AD4853/AD4855/AD4857, 20-bit for
      AD4852/AD4854/AD4856/AD4858
...
+---------------+----------+----------------+
| cfg_cmos_rand | Random   | 0 (CMOS)       |
+---------------+----------+----------------+
| cfg_lvds_rand | Random   | 1 (LVDS)       |
+---------------+----------+----------------+
...
Test program       Usage
================== =========================
test_program_cmos  Tests the CMOS interface
test_program_lvds  Tests the LVDS interface
================== =========================
...
-  os_en: Enables oversampling mode.
   Options: 0 - Disabled, 1 - Enabled

These parameters are randomized at build time in the numbered configuration
files (cfg1-cfg8), not in the random configurations. The valid range for
``packet_format`` depends on the device resolution: 0-1 for 16-bit devices
(AD4851/3/5/7) and 0-3 for 20-bit devices (AD4852/4/6/8). The ``crc_en``
and ``os_en`` parameters are randomized as 0 or 1 for all devices.

When using these configurations, the test program runs a single test case with
the randomized values.

For the random configurations (``cfg_cmos_rand``, ``cfg_lvds_rand``), these
parameters are not set at build time. Instead, the test program internally
generates all valid combinations of ``packet_format``, ``crc_en``, and
``os_en``, then randomly selects 5 to 10 test cases to run.

.. warning::

   **CMOS interface limitation:** The CRC check might break the captured data
   when using max sample rate with scki = 100MHz and packet format >= 20 bit.
   The data transaction doesn't meet the minimum timing requirement between the
   last SCKI edge and the CNV signal according to the datasheet.
