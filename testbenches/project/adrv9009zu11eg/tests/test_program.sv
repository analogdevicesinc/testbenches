...
import custom_test_harness_env_pkg::*;
...
//import `PKGIFY(test_harness, ext_ddr_0_axi_vip)::*;
import `PKGIFY(test_harness, ext_ddr_1_axi_vip)::*;
...
program test_program;
  ...
  custom_test_harness_env base_env;
  adi_axi_master_agent #(`AXI_VIP_PARAMS(test_harness, mng_axi_vip)) mng;
  adi_axi_slave_mem_agent #(`AXI_VIP_PARAMS(test_harness, ddr_axi_vip)) ddr;
  //adi_axi_slave_mem_agent #(`AXI_VIP_PARAMS(test_harness, ext_ddr_0_axi_vip)) ext_ddr_0;
  adi_axi_slave_mem_agent #(`AXI_VIP_PARAMS(test_harness, ext_ddr_1_axi_vip)) ext_ddr_1;
  bit [31:0] val;
  ...
  initial begin
    ...
    base_env = new(
      ...
      .ext_ddr_clk_vip_if(`TH.`EXT_DDR_CLK.inst.IF),
    ...
    //ext_ddr_0 = new("", `TH.`EXT_DDR_0_AXI.inst.IF);
    ext_ddr_1 = new("", `TH.`EXT_DDR_1_AXI.inst.IF);

    `LINK(mng, base_env, mng)
    `LINK(ddr, base_env, ddr)
    //`LINK(ext_ddr_0, base_env, ext_ddr_0)
    `LINK(ext_ddr_1, base_env, ext_ddr_1)
    ...
    `TH.`EXT_DDR_CLK.inst.IF.start_clock();
    ...
    `TH.`EXT_DDR_CLK.inst.IF.stop_clock();
  ...
  task rx_tpl_test(int use_dds);
    for (int i = 0; i < `TX_JESD_M; i++) begin
      if (use_dds) begin
        ...
        if (i < `TX_JESD_M/2) begin
          // Set DMA as source for DAC TPL
          base_env.mng.master_sequencer.RegWrite32(`EX_DAC_TPL_BA+'h40*i+GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_7),
                             `SET_DAC_CHANNEL_REG_CHAN_CNTRL_7_DAC_DDS_SEL(2));
        end else begin
          // Disable source for DAC TPL
          base_env.mng.master_sequencer.RegWrite32(`EX_DAC_TPL_BA+'h40*i+GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_7),
                             `SET_DAC_CHANNEL_REG_CHAN_CNTRL_7_DAC_DDS_SEL(3));
        end
      end
    end

    for (int i = 0; i < `RX_JESD_M; i++) begin
      if (i < `RX_JESD_M/2) begin
    ...
    // Configure RX DO
    base_env.mng.master_sequencer.RegWrite32(`RX_OFFLOAD_BA+'h1C, 32'h000001FF);
    ...
    if (!use_dds) begin
      ...
      base_env.mng.master_sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_FLAGS),
                         `SET_DMAC_FLAGS_TLAST(1));
      base_env.mng.master_sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_IRQ_PENDING),
                         `SET_DMAC_IRQ_PENDING_TRANSFER_COMPLETED(1) |
                         `SET_DMAC_IRQ_PENDING_TRANSFER_QUEUED(1));
      base_env.mng.master_sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_IRQ_MASK),
      ...
      base_env.mng.master_sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_X_LENGTH),
                         `SET_DMAC_X_LENGTH_X_LENGTH(32'h000031FF));
      ...
      @(posedge `TH.axi_adrv9009_som_rx_dma.irq);
      base_env.mng.master_sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_IRQ_PENDING),
                         `SET_DMAC_IRQ_PENDING_TRANSFER_COMPLETED(1));
      base_env.mng.master_sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_TRANSFER_SUBMIT),
                         `SET_DMAC_TRANSFER_SUBMIT_TRANSFER_SUBMIT(1));
