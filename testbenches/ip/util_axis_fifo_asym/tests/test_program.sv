    ...
    repeat($urandom_range(5,10)) begin
      ...
      if ((!`TKEEP_EN || !`TLAST_EN) && `INPUT_WIDTH < `OUTPUT_WIDTH) begin
        repeat($urandom_range(1,5)) begin
          uaf_env.input_axis_agent.master_sequencer.add_xfer_transaction_sample_count($urandom_range(1,128)*`OUTPUT_WIDTH/`INPUT_WIDTH, `TLAST_EN, 0);
        end
      end else begin
        repeat($urandom_range(1,5)) begin
          uaf_env.input_axis_agent.master_sequencer.add_xfer_transaction_byte_count($urandom_range(1,1024), `TLAST_EN, 0);
        end
      end

      #($urandom_range(1,10)*1us);

      uaf_env.input_axis_agent.master_sequencer.clear_transaction_queue();
      uaf_env.input_axis_agent.master_sequencer.wait_empty_transaction_queue();
