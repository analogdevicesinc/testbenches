...
// Simulate AD7175-2 DOUT/RDY behavior: when the SPI bus is idle (no SCLK
// activity), drive SDI LOW to signal conversion complete. In hardware, the
// ADC autonomously pulls DOUT/RDY LOW when a conversion finishes. This lets
// util_sigma_delta_spi naturally detect data_ready through its 63-clock
// idle counter and sdi_d pipeline check.
bit sclk_prev = 0;
bit [3:0] sclk_idle_cnt = 0;
wire bus_idle = (sclk_idle_cnt >= SCLK_IDLE_THRESHOLD);

initial begin
  forever begin
    @(posedge cn0363_sys_clk);
    sclk_prev <= cn0363_spi_sclk;
    if (sclk_prev != cn0363_spi_sclk) begin
      sclk_idle_cnt <= 0;
    end else if (sclk_idle_cnt < SCLK_IDLE_THRESHOLD) begin
      sclk_idle_cnt <= sclk_idle_cnt + 1;
    end
  end
end

assign cn0363_spi_sdi = bus_idle ? 1'b0 : sdi_shiftreg[31];
...
task offload_spi_test();
    ...
    // Assert CS LOW before enabling offload to bootstrap the first trigger
    spi_api.fifo_command(.cmd(`SET_CS(8'hFE)));
    #200ns;
