...
:git-hdl:`projects/adrv9009` reference design.
...
along with JESD and DMA exercisers and VIPs used for clocking, reset, PS and
DDR simulations.
...
ex_rx_axi_dma                   0x7C45_0000
ex_tx_axi_dma                   0x7C46_0000
ex_tx_os_axi_dma                0x7C47_0000
...
* Instantiate and initialize AXI-accessible verification components that model
  both the DUT and the exerciser (EX) interfaces. Each component is connected
  to the common AXI master sequencer within the base environment and linked to
  its corresponding JESD link configuration object.
...
* Transceiver clock setup
   - Exerciser (ex_*) and DUT (dut_*) transceivers are configured via
...
* **Enable exerciser ADC TPL channels**
...
* **Initialize DMA-based stimulus (if use_dds == 0)**
   ...
   - Configure the EX RX DMA engine by writing to its registers:

     - Enable DMA.
     - Set TLAST flag. 
     - Set transfer length.
     - Set destination address to DDR base.
     - Submit DMA transfer.

   - Wait for 5 µs to ensure the TX DMA starts fetching data.

* **Bring up the DUT TX path and exerciser RX path**

   - Start the TX clock generator (`AXI_CLKGEN_TX_BA`) and bring up the DUT TX
     transceiver.
   - Bring up the exerciser RX transceiver and link.
   - Wait for both DUT and exerciser link layers to report link-up status.

* **Simulation delay**

   - Wait for 5 µs to allow data flow and link stabilization.
...
* **Shutdown the links**
   ...
   - Shutdown the DMAs and transceivers to conclude the test.

RX TPL test
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* **Configure Exerciser DAC TPL channels**
...
* **Release DAC and ADC Resets**
   ...
   - Deassert reset (`RSTN = 1`) for both the exerciser DAC TPL and ADC TPL
     common blocks.
...
* **Initialize DMA-based stimulus (if use_dds == 0)**
   ...
   - Write a deterministic pattern to DDR memory using the ``BackdoorWrite32`` 
     interface of the DDR slave sequencer.
   - Configure the EX TX DMA engine by writing to its registers:
   ...
   - Configure the RX DMA engine by writing to its registers:
   ...
   - Wait for 5 µs to ensure the EX TX DMA starts fetching data.

* **Bring up DUT RX and Exerciser TX Paths**

   - Start the RX clock generator (`AXI_CLKGEN_RX_BA`) for the DUT.
   - Bring up the exerciser TX transceiver and link.
   - Bring up the DUT RX transceiver and link.
   - Wait for both DUT and exerciser link layers to report link-up.
...
* **Configure Exerciser DAC OS TPL Channels**
...
* **Release DAC and ADC OS Resets**
   ...
   - Deassert reset (`RSTN = 1`) for both the exerciser DAC OS TPL and ADC OS TPL
     common blocks.
...
* **Initialize DMA-based stimulus (if use_dds == 0)**
   ...
   - Write a deterministic pattern to DDR memory using the ``BackdoorWrite32`` 
     interface of the DDR slave sequencer.
   - Configure the EX TX OS DMA engine by writing to its registers:
   ...
   - Configure the RX OS DMA engine by writing to its registers:
   ...
   - Wait for 5 µs to ensure the EX TX OS DMA starts fetching data.

* **Bring up DUT RX Observation and Exerciser TX Observation Paths**

   - Start the RX OS clock generator (`AXI_CLKGEN_RX_OS_BA`) for the DUT.
   - Bring up the exerciser TX OS transceiver and link.
   - Bring up the DUT RX OS transceiver and link.
   - Wait for both DUT and exerciser RX OS links to report link-up.

* **Simulation Delay**

   - Wait 5 µs for data flow and link stabilization.
