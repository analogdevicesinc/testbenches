...
task fifo_init_test();
  ...
  `INFO(("Wait for SPI VIP receiving data"));
  for (int i = 0; i<(`NUM_OF_WORDS) ; i=i+1) begin
...
task fifo_double_write_test();
  ...
  if (sdo_2_fifo_data !== sdo_2_fifo_data_store) begin
    `INFOV(("sdo_2_fifo_data: %x; sdo_2_fifo_data_store %x", sdo_2_fifo_data, sdo_2_fifo_data_store),6);
    `ERROR(("Double Write Test FAILED"));
  end
  `INFO(("Double Write Test PASSED"));
endtask
...
task fifo_double_read_test();
  ...
  `INFO(("Wait for SPI VIP data send"));
  spi_wait_send();
  `INFO(("SPI sent"));
  ...
  if (sdi_2_fifo_data !== sdi_2_fifo_data_store) begin
    `INFOV(("sdi_2_fifo_data: %x; sdi_2_fifo_data_store %x", sdi_2_fifo_data, sdi_2_fifo_data_store),6);
    `ERROR(("Double Read Test FAILED"));
  end
  `INFO(("Double Read Test PASSED"));
endtask
...
task fifo_single_read_test();
  ...
  if (sdi_1_fifo_data !== sdi_1_fifo_data_store) begin
    `INFOV(("sdi_1_fifo_data: %x; sdi_1_fifo_data_store %x", sdi_1_fifo_data, sdi_1_fifo_data_store),6);
    `ERROR(("Single Read Test FAILED"));
  end
  `INFO(("Single Read Test PASSED"));
endtask
