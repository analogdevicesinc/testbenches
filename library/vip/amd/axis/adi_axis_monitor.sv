...
package adi_axis_monitor_pkg;
  ...
  class adi_axis_monitor #(`AXIS_VIP_PARAM_DECL(AXIS)) extends adi_axis_monitor_base;
    ...
    virtual task get_transaction();
      ...
      xil_axi4stream_data_byte tdata[AXIS_VIP_DATA_WIDTH/8-1:0];
      xil_axi4stream_strb tkeep[AXIS_VIP_DATA_WIDTH/8-1:0];
      xil_axi4stream_strb tstrb[AXIS_VIP_DATA_WIDTH/8-1:0];
      bit tlast;
      ...
      forever begin
        ...
        adi_transaction.tlast = tlast;
        adi_transaction.tid = tid;
        adi_transaction.tdest = tdest;
        if (!AXIS_VIP_USER_BITS_PER_BYTE) begin
          adi_transaction.tuser = tuser;
        end
        for (int i=0; i<AXIS_VIP_DATA_WIDTH/8; i++) begin
          adi_transaction.bytes[i].tdata = tdata[i];
          adi_transaction.bytes[i].tkeep = tkeep[i];
          adi_transaction.bytes[i].tstrb = tstrb[i];
          if (AXIS_VIP_USER_BITS_PER_BYTE) begin
            adi_transaction.bytes[i].tuser = tuser[i*AXIS_VIP_USER_WIDTH/(AXIS_VIP_DATA_WIDTH/8)+:AXIS_VIP_USER_WIDTH/(AXIS_VIP_DATA_WIDTH/8)];
          end
        end
