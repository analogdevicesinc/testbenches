...
program test_program (
    ...
    for (int i = 0, k = 0; i < ((`NUM_OF_TRANSFERS)*(`NUM_OF_WORDS)*(`NUM_OF_MISO)); i++) begin
      if (sdi_lane_mask[i%(`NUM_OF_MISO)]) begin
        ...
        if (sdi_read_data[k] != sdi_read_data_store[k]) begin //one word at a time comparison
          `INFO(("sdi_read_data[%0d]: %x; sdi_read_data_store[%0d]: %x",
          k, sdi_read_data[k],
  ...
  task fifo_spi_test();
    ...
    // Generate random contiguous lane masks
    mask_width                 = $urandom_range(`NUM_OF_MISO, 1);
    sdi_lane_mask              = ((1 << mask_width) - 1) << $urandom_range(`NUM_OF_MISO - mask_width, 0);
    mask_width                 = $urandom_range(`NUM_OF_MOSI, 1);
    sdo_lane_mask              = ((1 << mask_width) - 1) << $urandom_range(`NUM_OF_MOSI - mask_width, 0);

    //This test forces a wrong lane mask (complement of actual), then generates data
    //and much time later starts execution with the correct lane mask
    spi_api.fifo_command(`SET_SDI_LANE_MASK(~sdi_lane_mask)); //wrong lane mask on purpose
    spi_api.fifo_command(`SET_SDO_LANE_MASK(~sdo_lane_mask)); //wrong lane mask on purpose
    num_of_active_sdi_lanes = $countones(sdi_lane_mask);
    num_of_active_sdo_lanes = $countones(sdo_lane_mask);
    ...
    foreach (sdi_fifo_data[i]) begin
      if (sdi_fifo_data[i] !== sdi_fifo_data_store[i]) begin
        `INFO(("sdi_fifo_data[%0d]: %x; sdi_fifo_data_store[%0d]: %x", i, sdi_fifo_data[i], i, sdi_fifo_data_store[i]), ADI_VERBOSITY_LOW);
    ...
    foreach (sdo_fifo_data[i]) begin
      if (sdo_fifo_data[i] !== sdo_fifo_data_store[i]) begin
        `INFO(("sdo_fifo_data[%0d]: %x; sdo_fifo_data_store[%0d]: %x", i, sdo_fifo_data[i], i, sdo_fifo_data_store[i]), ADI_VERBOSITY_LOW);
