...
task automatic run_verification_suite();
  ...
  // check.
  if (`NUM_OF_WORDS > 1)
    tput_meter.verify_throughput(
      .expected_per_ch_ksps(TARGET_KSPS_PER_CH),
      .tol_pct(TARGET_KSPS_TOL_PCT),
      .error_cnt(total_error_count)
    );
  else
    tput_meter.verify_throughput(
      .expected_per_ch_ksps(0.0),
      .tol_pct(0.0),
      .error_cnt(total_error_count)
    );
  ...
  verify_received_data(
    .word_cnt(`NUM_OF_TRANSFERS * `NUM_OF_WORDS),
    .error_cnt(total_error_count)
  );
endtask
...
initial begin : irq_callback
  forever begin : irq_callback_loop
    ...
    // Decode via named regmap fields (IRQ_PENDING shares the IRQ_MASK bit layout)
...
// Expected words, generated by generate_sdo_data and checked in
// verify_received_data. Received words are compared against this.
