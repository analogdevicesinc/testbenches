...
-  DATA_DLENGTH: defines the actual data length in bits being transferred
...
The following configuration file is available:
   ...
   | mode            +--------------+-------------+----------+------------+------+------+----------------+--------------+----------------+
   |                 | DATA_WIDTH   | DATA_DLENGTH| NUM_OF_CS| THREE_WIRE | CPOL | CPHA | CLOCK_DIVIDER  | NUM_OF_WORDS | SDO_IDLE_STATE |
   +=================+==============+=============+==========+============+======+======+================+==============+================+
   | cfg1            | 8            | 8           | 2        | 0          | 0    | 1    | 0              | 1            | 0              |
   +-----------------+--------------+-------------+----------+------------+------+------+----------------+--------------+----------------+
...
The testbench simulates the AD7175-2 DOUT/RDY behavior by driving SDI LOW
when the SPI bus is idle, allowing util_sigma_delta_spi to naturally detect
data_ready and trigger offload transfers.
...
* Configure the Offload module with SPI Engine command sequence:
  ...
  * De-assert chip select
  * Re-assert chip select (re-arms the 63-clock data_ready counter)
  * SYNC command with ID=2 to generate interrupt

* Assert CS LOW via FIFO command to bootstrap the first natural trigger
