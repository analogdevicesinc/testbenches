...
program test_program_3;

  timeunit 1ns;
  timeprecision 1ps;

  custom_test_harness_env #(`AXI_VIP_PARAMS(test_harness, mng_axi_vip), `AXI_VIP_PARAMS(test_harness, ddr_axi_vip), `AXI_VIP_PARAMS(test_harness, ext_ddr_0_axi_vip), `AXI_VIP_PARAMS(test_harness, ext_ddr_1_axi_vip)) base_env;
  bit [31:0] val;
  ...
  initial begin
    ...
    base_env = new("Base Environment",
                    ...
                    `TH.`DMA_CLK.inst.IF,
                    `TH.`DDR_CLK.inst.IF,
                    `TH.`EXT_DDR_CLK.inst.IF,
                    `TH.`SYS_RST.inst.IF,
                    `TH.`MNG_AXI.inst.IF,
                    `TH.`DDR_AXI.inst.IF,
                    `TH.`EXT_DDR_0_AXI.inst.IF,
                    `TH.`EXT_DDR_1_AXI.inst.IF);
  ...
  task rx_tpl_test(int use_dds);
    ...
    // Configure RX DO
    base_env.mng.sequencer.RegWrite32(`RX_OFFLOAD_BA+'h1C, 32'h000001FF);
    base_env.mng.sequencer.RegWrite32(`RX_OFFLOAD_BA+'h88, 32'h00000000);
    ...
    if (!use_dds) begin
      ...
      base_env.mng.sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_FLAGS),
                         `SET_DMAC_FLAGS_TLAST(1));
      base_env.mng.sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_IRQ_PENDING),
                         `SET_DMAC_IRQ_PENDING_TRANSFER_COMPLETED(1) |
                         `SET_DMAC_IRQ_PENDING_TRANSFER_QUEUED(1));
      base_env.mng.sequencer.RegWrite32(`RX_DMA_BA+GetAddrs(DMAC_IRQ_MASK),
