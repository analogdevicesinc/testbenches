...
task sdo_stream_gen(
  ...
  for (int i = 0; i<(`DATA_WIDTH/8);i++) begin
    ...
    env.sdo_src_agent.sequencer.push_byte_for_stream(data[i]);
  end
  env.sdo_src_agent.sequencer.add_xfer_descriptor((`DATA_WIDTH/8),0,0);
  `endif
endtask
...
task offload_spi_test();
  ...
  for (int i=0; i<=((`NUM_OF_TRANSFERS)*(`NUM_OF_WORDS) -1); i=i+1) begin
    sdi_read_data[i] = env.ddr.agent.mem_model.backdoor_memory_read_4byte(`DDR_BA + 4*i);
