...
program test_program_cmos (
  ...
  // localparam T_CONV_NS = 665; // T_CONV typ din datasheet
  // localparam T_OS_CONV = 4 * T_CONV_NS;

  reg         [31:0]      rx_db_i[0:8];
  reg         [ 5:0]      db_i_index = 0;
  reg         [ 3:0]      ring_buffer_index = 0;

  wire        [ 3:0]      ch_index_lane_0;
  wire        [ 3:0]      ch_index_lane_1;
  wire        [ 3:0]      ch_index_lane_2;
  wire        [ 3:0]      ch_index_lane_3;
  wire        [ 3:0]      ch_index_lane_4;
  wire        [ 3:0]      ch_index_lane_5;
  wire        [ 3:0]      ch_index_lane_6;
  wire        [ 3:0]      ch_index_lane_7;
  reg         [ 7:0]      db_i_shift = 0;

  reg                     oversampling = 0; //trebuie scrise in ADC_COMMON prin axi
  reg         [1:0]       packet_format = 2; // 0 - 20 bits, 1 - 24 bits, 2 - 32 bits; 16bits la ad4851- cum fac?
  reg                     crc_en = 1;
  ...
  task configure_axi();
    ...
    `INFO(("adc_custom_ctrl: %x", adc_custom_ctrl), ADI_VERBOSITY_LOW);
    // axi_write (`AXI_AD485X_BA + GetAddrs(ADC_COMMON_REG_CNTRL_3), `SET_ADC_COMMON_REG_CNTRL_3_CUSTOM_CONTROL(adc_custom_ctrl)); //bit[2] = 0 - no oversampling; bits[1:0] = 0 - pachet cu 20bits - axi ad4858 de pe wiki
    // axi_write (`AXI_AD485X_BA + GetAddrs(ADC_COMMON_REG_CNTRL_3), `SET_ADC_COMMON_REG_CNTRL_3_CRC_EN(crc_en));//no crc
    // axi_write (`AXI_AD485X_BA + GetAddrs(ADC_COMMON_REG_CNTRL), `SET_ADC_COMMON_REG_CNTRL_NUM_LANES(NUMB_OF_CH));

// Combine both fields in a single write to avoid overwriting
  ...
  task enable_adc_ch();
  //Enable ADC channels
    `INFO(("Start Enabled channels."), ADI_VERBOSITY_LOW);
    ...
    `INFO(("end Enabled channels."), ADI_VERBOSITY_LOW);
  endtask;

  task config_test();
    // Start ADC clk generator
    `INFO(("start clkgen"), ADI_VERBOSITY_LOW);
    clkgen_api.enable_clkgen();
    `INFO(("end clkgen"), ADI_VERBOSITY_LOW);
    //#500ns
    `INFO(("start dma"), ADI_VERBOSITY_LOW);
    dma_api.enable_dma();
    dma_api.set_flags(
      .cyclic(1'b0),
      .tlast(1'b1),
    ...
    dma_api.set_lengths(
      .xfer_length_x((NUMB_OF_CH * 32 / 8) - 1), // Number of bytes to transfer - 1, numb of ch*32/8 -1 => 35 bytes
      .xfer_length_y(32'h0));
      dma_api.set_dest_addr(xil_axi_uint'(`DDR_BA));
    dma_api.transfer_start();
    `INFO(("start pwm"), ADI_VERBOSITY_LOW);
...
task prepare_data();
  `INFO(("start prepare data."), ADI_VERBOSITY_LOW);
  case (packet_format)
    0 : packet_sz <= 20;
    1 : packet_sz <= 24;
    2 : packet_sz <= 32;
  ...
  `INFO(("end prepare data."), ADI_VERBOSITY_LOW);
...
initial begin
  forever begin
      ...
      if (busy_d & !busy) begin //negedge
          ...
          db_i_shift[0] <= rx_db_i[ch_index_lane_0][db_i_index];
          db_i_shift[1] <= rx_db_i[ch_index_lane_1][db_i_index];
          db_i_shift[2] <= rx_db_i[ch_index_lane_2][db_i_index];
          db_i_shift[3] <= rx_db_i[ch_index_lane_3][db_i_index];
          db_i_shift[4] <= rx_db_i[ch_index_lane_4][db_i_index];
          db_i_shift[5] <= rx_db_i[ch_index_lane_5][db_i_index];
          db_i_shift[6] <= rx_db_i[ch_index_lane_6][db_i_index];
          db_i_shift[7] <= rx_db_i[ch_index_lane_7][db_i_index];
        end
      end
    end
  end

  assign ch_index_lane_0 = (0 + ring_buffer_index) == MAXC_INDEX ? 0 : (0 + ring_buffer_index) > MAXC_INDEX ? (0 + ring_buffer_index) -8 : 0 + ring_buffer_index;
  assign ch_index_lane_1 = (1 + ring_buffer_index) == MAXC_INDEX ? 0 : (1 + ring_buffer_index) > MAXC_INDEX ? (1 + ring_buffer_index) -8 : 1 + ring_buffer_index;
  assign ch_index_lane_2 = (2 + ring_buffer_index) == MAXC_INDEX ? 0 : (2 + ring_buffer_index) > MAXC_INDEX ? (2 + ring_buffer_index) -8 : 2 + ring_buffer_index;
  assign ch_index_lane_3 = (3 + ring_buffer_index) == MAXC_INDEX ? 0 : (3 + ring_buffer_index) > MAXC_INDEX ? (3 + ring_buffer_index) -8 : 3 + ring_buffer_index;
  assign ch_index_lane_4 = (4 + ring_buffer_index) == MAXC_INDEX ? 0 : (4 + ring_buffer_index) > MAXC_INDEX ? (4 + ring_buffer_index) -8 : 4 + ring_buffer_index;
  assign ch_index_lane_5 = (5 + ring_buffer_index) == MAXC_INDEX ? 0 : (5 + ring_buffer_index) > MAXC_INDEX ? (5 + ring_buffer_index) -8 : 5 + ring_buffer_index;
  assign ch_index_lane_6 = (6 + ring_buffer_index) == MAXC_INDEX ? 0 : (6 + ring_buffer_index) > MAXC_INDEX ? (6 + ring_buffer_index) -8 : 6 + ring_buffer_index;
  assign ch_index_lane_7 = (7 + ring_buffer_index) == MAXC_INDEX ? 0 : (7 + ring_buffer_index) > MAXC_INDEX ? (7 + ring_buffer_index) -8 : 7 + ring_buffer_index;
...
// assign send_word[0] = rx_db_i[0];
// assign send_word[1] = rx_db_i[1];
// assign send_word[2] = rx_db_i[2];
// assign send_word[3] = rx_db_i[3];
// assign send_word[4] = rx_db_i[4];
// assign send_word[5] = rx_db_i[5];
// assign send_word[6] = rx_db_i[6];
// assign send_word[7] = rx_db_i[7];
// assign send_word[8] = rx_db_i[8];
