...
task generate_fifo_transfer_cmd(
    input [7:0] sync_id);
  // assert CSN
  spi_api.write_fifo_instr(`SET_CS(8'hFE));
  // transfer data
  spi_api.write_fifo_instr(`INST_WRD);
  // de-assert CSN
...
task offload_spi_test();
  ...
  spi_api.write_offload_instr(`INST_WRD);
  ...
  `else
      if (i<(`NUM_OF_WORDS)) begin
        sdo_write_data_store[i] = tx_data;
        spi_api.write_offload_sdo_fifo(sdo_write_data_store[i]);
      end else begin
...
task fifo_spi_test();
  ...
  env.mng.RegWrite32(`SPI_ENGINE_PWM_GEN_BA + GetAddrs(AXI_PWM_GEN_REG_PULSE_X_PERIOD), `SET_AXI_PWM_GEN_REG_PULSE_X_PERIOD_PULSE_X_PERIOD('d121)); // set PWM period
  ...
  for (int i = 0; i<(`NUM_OF_WORDS) ; i=i+1) begin
    rx_data = $urandom;
    tx_data = $urandom;
    spi_api.write_sdo_fifo(tx_data);
    spi_vip_send(rx_data);
    sdi_fifo_data_store[i] = rx_data;
    sdo_fifo_data_store[i] = tx_data;
  end

  generate_fifo_transfer_cmd(1);
  ...
  for (int i = 0; i<(`NUM_OF_WORDS) ; i=i+1) begin
    spi_api.read_sdi_fifo(sdi_fifo_data[i]);
    spi_vip_receive(sdo_fifo_data[i]);
  end
