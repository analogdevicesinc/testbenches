...
package adi_axis_frame_pkg;
  ...
  class adi_axis_frame #(
    ...
    adi_axis_packet #(BYTES_PER_TRANSACTION, EN_TKEEP, EN_TSTRB, EN_TUSER, EN_TID, EN_TDEST, TUSER_BYTE_BASED, TID_WIDTH, TDEST_WIDTH, TUSER_WIDTH) packets[];
    ...
    function void create_frame(input int frame_size);
      ...
      if (this.packet_size_limit != 0) begin
        for (int i=0; i<this.packets.size(); i++) begin
          this.packets[i] = new(.packet_size_limit(this.packet_size_limit));
        end
      end
    ...
    function void update_packet_class(
      input int location,
      input adi_axis_packet #(BYTES_PER_TRANSACTION, EN_TKEEP, EN_TSTRB, EN_TUSER, EN_TID, EN_TDEST, TUSER_BYTE_BASED, TID_WIDTH, TDEST_WIDTH, TUSER_WIDTH) packet_info);

      packet_info.copy(this.packets[location]);
    endfunction: update_packet_class

    function void add_packet_class(input adi_axis_packet #(BYTES_PER_TRANSACTION, EN_TKEEP, EN_TSTRB, EN_TUSER, EN_TID, EN_TDEST, TUSER_BYTE_BASED, TID_WIDTH, TDEST_WIDTH, TUSER_WIDTH) packet_info);
    ...
    virtual function void do_copy(input adi_object object);
      adi_axis_frame #(BYTES_PER_TRANSACTION, EN_TKEEP, EN_TSTRB, EN_TUSER, EN_TID, EN_TDEST, TUSER_BYTE_BASED, TID_WIDTH, TDEST_WIDTH, TUSER_WIDTH) cast_object;
