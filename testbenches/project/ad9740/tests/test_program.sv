...
// configures the DAC_RESOLUTION in the axi_ad9740 IP. The DEVICE define is
// used to determine the resolution for verification.
//
// ***************************************************************************
...
task axi_write(input [31:0] addr, input [31:0] data);
  base_env.mng.master_sequencer.RegWrite32(addr, data);
endtask

task axi_read(input [31:0] addr, output [31:0] data);
  base_env.mng.master_sequencer.RegRead32(addr, data);
endtask

task axi_read_v(input [31:0] addr, input [31:0] data);
  base_env.mng.master_sequencer.RegReadVerify32(addr, data);
endtask
...
task write_dma_data_to_ddr();
  ...
  for (int i = 0; i < TEST_SAMPLES; i += 2) begin
    ...
    base_env.ddr.slave_sequencer.BackdoorWrite32(.addr(DDR_BASE + i*2), .data(data32), .strb('1));
  end
endtask
...
task init_dac();
  ...
  // Reset sequence
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_COMMON_REG_RSTN), 32'h0);
  #500ns;
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_COMMON_REG_RSTN), 32'h3);
  #500ns;
  ...
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_COMMON_REG_CNTRL_2), 32'h00000000);  // dac_datafmt = 0 (pass-through)

  // Read version
  axi_read(AD974X_DAC_BA + GetAddrs(COMMON_REG_VERSION), regval);
  `INFO(($sformatf("DAC Version: 0x%08x", regval)), ADI_VERBOSITY_NONE);

  // Scratch register sanity check
  axi_write(AD974X_DAC_BA + GetAddrs(COMMON_REG_SCRATCH), 32'hA5A5A5A5);
  axi_read_v(AD974X_DAC_BA + GetAddrs(COMMON_REG_SCRATCH), 32'hA5A5A5A5);
  axi_write(AD974X_DAC_BA + GetAddrs(COMMON_REG_SCRATCH), 32'h00000000);
  axi_read_v(AD974X_DAC_BA + GetAddrs(COMMON_REG_SCRATCH), 32'h00000000);
endtask
...
task set_data_source(input [3:0] source);
  ...
  // Only bits [3:0] are used for dac_data_sel
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_7), {28'h0, source});
...
task start_dma_transfer(input [31:0] src_addr, input [31:0] length);
  ...
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_CONTROL), 32'h0);
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_CONTROL), 32'h1);
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_FLAGS), 32'h1);
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_SRC_ADDRESS), src_addr);
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_X_LENGTH), length - 1);
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_Y_LENGTH), 32'h0);
  axi_write(AD974X_DMA_BA + GetAddrs(DMAC_TRANSFER_SUBMIT), 32'h1);
endtask
...
task capture_dac_output(input bit [15:0] num_samples);
  ...
  `INFO(($sformatf("Capturing %0d DAC output samples (DDR both edges)", num_samples)), ADI_VERBOSITY_NONE);

  for (int i = 0; i < num_samples; i++) begin
    @(posedge ad974x_clk or negedge ad974x_clk);
...
task verify_dma_datapath();
  ...
  automatic int skip = 16;  // skip initial samples for pipeline settling
  automatic int phase_offset = -1;

  `INFO(("Verifying DMA data path..."), ADI_VERBOSITY_NONE);

  if (captured_data.size() < verify_samples) begin
    `ERROR(($sformatf("Insufficient samples: %0d < %0d", captured_data.size(), verify_samples)));
    return;
  end

  // Find phase offset: match captured data against cyclic DMA buffer
  begin
    automatic bit [15:0] target_dac = captured_data[skip] >> internal_shift;
    for (int j = 0; j < TEST_SAMPLES; j++) begin
      automatic bit [15:0] candidate = dma_data[j] >> dma_shift;
      if ((candidate > target_dac ? candidate - target_dac : target_dac - candidate) <= TOLERANCE) begin
        // Confirm match with next 3 consecutive samples
        automatic bit match = 1;
        for (int k = 1; k < 4; k++) begin
          automatic bit [15:0] exp_k = dma_data[(j+k) % TEST_SAMPLES] >> dma_shift;
          automatic bit [15:0] act_k = captured_data[skip+k] >> internal_shift;
          if ((act_k > exp_k ? act_k - exp_k : exp_k - act_k) > TOLERANCE) begin
            match = 0;
            break;
          end
        end
  ...
  if (phase_offset == -1) begin
    // Dump first samples for debug
    for (int i = 0; i < 8 && i < captured_data.size(); i++) begin
      `INFO(($sformatf("  captured[%0d] = 0x%04x (DAC: 0x%03x)", i, captured_data[i], captured_data[i] >> internal_shift)), ADI_VERBOSITY_NONE);
    end
    `ERROR(("Could not find DMA data phase offset"));
    return;
  end

  `INFO(($sformatf("  DMA phase offset: %0d", phase_offset)), ADI_VERBOSITY_NONE);

  // Verify samples with correct phase alignment
  for (int i = skip; i < verify_samples && i < captured_data.size(); i++) begin
    automatic int dma_idx = (phase_offset + (i - skip)) % TEST_SAMPLES;
    expected_dac = dma_data[dma_idx] >> dma_shift;
    actual_dac = captured_data[i] >> internal_shift;

    if (i < skip + 8) begin
      `INFO(($sformatf("  [%3d]: DMA[%3d]=0x%04x -> Exp=0x%03x, Act=0x%03x",
                       i, dma_idx, dma_data[dma_idx], expected_dac, actual_dac)), ADI_VERBOSITY_NONE);
    end

    if ((actual_dac > expected_dac ? actual_dac - expected_dac : expected_dac - actual_dac) > TOLERANCE) begin
      local_errors++;
      if (local_errors <= 5) begin
        `INFO(($sformatf("  MISMATCH [%3d]: DMA[%3d] Exp=0x%03x, Act=0x%03x",
                         i, dma_idx, expected_dac, actual_dac)), ADI_VERBOSITY_NONE);
      end
    end
  end
  ...
  if (local_errors == 0) begin
    ...
    `ERROR(($sformatf("DMA data path: FAILED (%0d errors)", local_errors)));
  end
endtask
...
task verify_ramp_datapath();
  ...
  if (captured_data.size() < check_samples) begin
    `ERROR(($sformatf("Insufficient ramp samples: %0d < %0d", captured_data.size(), check_samples)));
    return;
  end
  ...
  if (local_errors == 0 && valid_transitions > check_samples/2) begin
    ...
    `ERROR(($sformatf("RAMP data path: FAILED (%0d errors, %0d valid)", local_errors, valid_transitions)));
  end
endtask
...
task test_dds_mode();
  ...
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_1), 32'h00004000);  // scale_1 = 0x4000 (half scale)
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_2), 32'h00000100);  // init_1=0, incr_1=0x0100

  // Disable tone 2 (scale=0)
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_3), 32'h00000000);  // scale_2 = 0
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_4), 32'h00000000);  // init_2=0, incr_2=0

  // Step 3: Select DDS as data source (data_sel = 0)
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_CHANNEL_REG_CHAN_CNTRL_7), 32'h00000000);  // data_sel = 0 (DDS)
  ...
  if (`DAC_DATAFMT == 1) begin
    axi_write(AD974X_DAC_BA + GetAddrs(DAC_COMMON_REG_CNTRL_2), 32'h00000000);  // dac_datafmt = 0 (pass-through signed)
  end else begin
    axi_write(AD974X_DAC_BA + GetAddrs(DAC_COMMON_REG_CNTRL_2), 32'h00000010);  // dac_datafmt = 1 (convert to unsigned)
  end
  ...
  axi_write(AD974X_DAC_BA + GetAddrs(DAC_COMMON_REG_CNTRL_1), 32'h1);  // sync = 1
  #1us;
  ...
  if (nonzero_samples > captured_data.size() / 2) begin
    ...
    `ERROR(($sformatf("DDS mode: FAILED (%0d/%0d non-zero)", nonzero_samples, captured_data.size())));
  end
endtask
...
initial begin
  ...
  write_dma_data_to_ddr();
  start_dma_transfer(DDR_BASE, TEST_SAMPLES * 2);
  #15us;  // Wait for DMA pipeline to fill
  capture_dac_output(verify_samples + 16);
  verify_dma_datapath();
  ...
  `INFO((""), ADI_VERBOSITY_NONE);
  `INFO(("=========================================================="), ADI_VERBOSITY_NONE);
  `INFO(($sformatf("  %s (%s) DATA PATH VALIDATION: COMPLETE", device_name, data_format_name)), ADI_VERBOSITY_NONE);
  `INFO(("=========================================================="), ADI_VERBOSITY_NONE);
