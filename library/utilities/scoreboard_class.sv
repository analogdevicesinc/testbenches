...
package scoreboard_class_pkg;
  ...
  class scoreboard_class #(type data_type = null) extends scoreboard#(.data_type(data_type));

    class subscriber_class extends adi_subscriber #(data_type);

      protected scoreboard #(data_type) scoreboard_ref;

      protected adi_fifo_class #(data_type) data_fifo;
    ...
    virtual task compare_transaction();
      ...
      while ((this.subscriber_source.size() > 0) &&
        ...
        this.info($sformatf("Source-sink data: exp %s - rcv %s", source_data.sprint(), sink_data.sprint()), ADI_VERBOSITY_MEDIUM);
        if (source_data.compare(sink_data)) begin
          this.error($sformatf("Failed at: exp %s - rcv %s", source_data.sprint(), sink_data.sprint()));
        end
      end
